...
const Absences: React.FC = () => {
  ...
  const [filter, setFilter] = useState<'all' | 'reported' | 'unreported'>('all');
            ...
            <Button variant="outline" size="sm">
              ...
              <DropdownMenu>
                <DropdownMenuTrigger className="flex items-center">
                  {filter === 'all' ? 'All Absences' : 
                   filter === 'reported' ? 'Reported' : 'Unreported'}
                </DropdownMenuTrigger>
                <DropdownMenuContent align="end">
                  <DropdownMenuItem onClick={() => setFilter('all')}>All Absences</DropdownMenuItem>
                  <DropdownMenuItem onClick={() => setFilter('reported')}>Reported</DropdownMenuItem>
                  <DropdownMenuItem onClick={() => setFilter('unreported')}>Unreported</DropdownMenuItem>
                </DropdownMenuContent>
              </DropdownMenu>
            </Button>
          </div>
          ...
          <CardContent className="p-0">
            {filteredAbsences.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-12">
                <div className="text-muted-foreground mb-2">No absences found</div>
              </div>
            ) : (
              <Table>
                <TableHeader>
                ...
                <TableBody>
                  {filteredAbsences.map((absence) => (
                    <TableRow key={absence.id}>
                      ...
                      <TableCell>{absence.reason || 'No reason provided'}</TableCell>
                      <TableCell>
                        {absence.reported ? (
